...
folder = "data"
rawSamples = np.load(folder + "/Chain C - thinned 0.001.npy")
...
def onclick(event):
    ...
    ix = int(np.floor((event.xdata / dx - 25) / 5))
    iz = int(4 - np.floor((event.ydata / dx - 50) / 5))

    icor = ix + iz * 10 + (axis - 1) * 100
...
def drawBoxParameter(par):
    ...
    ix = block % 10
    iz = math.floor(block / 10)

    x_arr = (np.array([0, 5, 5, 0, 0]) + ix * 5 + 25) * dx
    z_arr = (np.array([0, 0, 5, 5, 0]) - iz * 5 + 70) * dx
